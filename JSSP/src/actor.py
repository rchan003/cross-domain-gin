...
from torch_geometric.nn import (  # NOTE: Edited here to use v2 GATConv
    GATv2Conv,
    GINConv,
    global_mean_pool,
)
...
class DGHANlayer(torch.nn.Module):
    def __init__(self, in_chnl, out_chnl, dropout, concat, heads=2):
        ...
        self.opsgrp_conv = GATv2Conv(
            in_chnl, out_chnl, heads=heads, dropout=dropout, concat=concat
        )
        self.mchgrp_conv = GATv2Conv(
            in_chnl, out_chnl, heads=heads, dropout=dropout, concat=concat
        )
    ...
    def forward(self, node_h, edge_index_pc, edge_index_mc):
        node_h_pc = F.elu(
            self.opsgrp_conv(
                F.dropout(node_h, p=self.dropout, training=self.training), edge_index_pc
            )
        )
        node_h_mc = F.elu(
            self.mchgrp_conv(
                F.dropout(node_h, p=self.dropout, training=self.training), edge_index_mc
            )
        )
...
class DGHAN(torch.nn.Module):
    def __init__(self, in_dim, hidden_dim, dropout, layer_dghan=4, heads=2):
        ...
        if layer_dghan == 1:
            # only DGHAN layer
            self.DGHAN_layers.append(
                DGHANlayer(in_dim, hidden_dim, dropout, concat=False, heads=heads)
            )
        else:
            # first DGHAN layer
            self.DGHAN_layers.append(
                DGHANlayer(in_dim, hidden_dim, dropout, concat=True, heads=heads)
            )
            ...
            self.DGHAN_layers.append(
                DGHANlayer(
                    heads * hidden_dim, hidden_dim, dropout, concat=False, heads=1
                )
            )
    ...
    def forward(self, x, edge_index_pc, edge_index_mc, batch_size):
        ...
        return h_node, torch.mean(
            h_node.reshape(batch_size, -1, self.hidden_dim), dim=1
        )
...
class Actor(nn.Module):
    def __init__(
        ...
        if self.embedding_type == "gin":
            self.embedding = GIN(
                in_dim=in_dim, hidden_dim=hidden_dim, layer_gin=embedding_l
            )
        elif self.embedding_type == "dghan":
            self.embedding = DGHAN(
                in_dim=in_dim,
            ...
            raise Exception(
                'embedding type should be either "gin", "dghan", or "gin+dghan".'
            )
    ...
    def forward(self, batch_states, feasible_actions):
        ...
        if self.embedding_type == "gin":
            node_embed, graph_embed = self.embedding(
                ...
                add_self_loops(
                    torch.cat(
                        [batch_states.edge_index_pc, batch_states.edge_index_mc], dim=-1
                    )
                )[0],
                batch_states.batch,
            )
        ...
        # action score
        action_score = torch.bmm(
            node_embed_augmented, node_embed_augmented.transpose(-1, -2)
        )
        ...
        for i in range(len(feasible_actions)):
            ...
            for j in range(len(feasible_actions[i])):
                a_merge.append(
                    [feasible_actions[i][j][0] + carries[i], feasible_actions[i][j][1]]
                )
        a_merge = np.array(a_merge)
        mask = torch.ones(
            size=[batch_size * n_nodes_per_state, n_nodes_per_state],
            dtype=torch.bool,
            device=device,
        )
        ...
        sampled_actions = [
            [
                actions_id[i].item() // n_nodes_per_state,
                actions_id[i].item() % n_nodes_per_state,
            ]
            for i in range(len(feasible_actions))
        ]
