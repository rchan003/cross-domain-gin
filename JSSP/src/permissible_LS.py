...
def permissibleLeftShift(h, a, durMat, mchMat, mchsStartTimes, opIDsOnMchs):
    jobRdyTime_a, mchRdyTime_a = __calJobAndMchRdyTimeOfa(
    ...
    if len(possiblePos) == 0:
        startTime_a = __putInTheEnd(
            h, a, jobRdyTime_a, mchRdyTime_a, startTimesForMchOfa, opsIDsForMchOfa
        )
    else:
        idxLegalPos, legalPos, endTimesForPossiblePos = __calLegalPos(
            dur_a,
            jobRdyTime_a,
            durMat,
            possiblePos,
...
def __putInTheEnd(
    ...
    # print('Yes!OK!')
    index = np.where(startTimesForMchOfa == -h)[0][0]
...
def __calLegalPos(
    dur_a, jobRdyTime_a, durMat, possiblePos, startTimesForMchOfa, opsIDsForMchOfa
):
    ...
    startTimeEarlst = max(
        ...
        + np.take(durMat, [opsIDsForMchOfa[possiblePos[0] - 1]]),
    )
...
def __calJobAndMchRdyTimeOfa(a, mchMat, durMat, mchsStartTimes, opIDsOnMchs):
    mch_a = np.take(mchMat, a) - 1
    # cal jobRdyTime_a
