...
import numpy as np
import torch
from src.message_passing_evl import CPM_batch_G, Evaluator
from src.permissible_LS import permissibleLeftShift
...
class JsspN5:
    ...
    def _adj_mat_pc(self):
        adj_mat_pc = np.eye(
            self.n_oprs, k=-1, dtype=int
        )  # Create adjacent matrix for precedence constraints
        adj_mat_pc[
            np.arange(start=0, stop=self.n_oprs, step=1).reshape(self.n_job, -1)[:, 0]
        ] = 0  # first column does not have upper stream conj_nei
        adj_mat_pc = np.pad(
            adj_mat_pc, 1, "constant", constant_values=0
        )  # pad dummy S and T nodes
        adj_mat_pc[
            [i for i in range(1, self.n_job * self.n_mch + 2 - 1, self.n_mch)], 0
        ] = 1  # connect S with 1st operation of each job
        adj_mat_pc[
            -1,
            [i for i in range(self.n_mch, self.n_job * self.n_mch + 2 - 1, self.n_mch)],
        ] = 1  # connect last operation of each job to T
        adj_mat_pc = np.transpose(
            adj_mat_pc
    ...
    def _gen_moves(self, solution, mch_mat, tabu_list=None):
        ...
        critical_blocks = mch_mat.take(
            critical_blocks_opr - 1
        )  # -1: ops id starting from 0
    ...
    def show_state(self, G):
        x_axis = np.pad(
            np.tile(np.arange(1, self.n_mch + 1, 1), self.n_job),
            (1, 1),
            "constant",
            constant_values=[0, self.n_mch + 1],
        )
        y_axis = np.pad(
            np.arange(self.n_job, 0, -1).repeat(self.n_mch),
            (1, 1),
            "constant",
    ...
    def _p_list_solver(self, args, plot=False):
        ...
        for i, (instance, priority_list) in enumerate(zip(instances, priority_lists)):
            ...
            # prepare NIPS adj
            ops_mat = (
                np.arange(0, n_operations).reshape(mch_mat.shape).tolist()
            )  # Init operations mat
            list_for_latest_task_onMachine = [
                None
            ] * n_machines  # Init list_for_latest_task_onMachine

            adj_mat_mc = np.zeros(
                shape=[n_operations, n_operations], dtype=int
            ...
            for job_id in priority_list:
                ...
                if list_for_latest_task_onMachine[m_id_for_action] is not None:
                    adj_mat_mc[
                        op_id, list_for_latest_task_onMachine[m_id_for_action]
                    ] = 1
                list_for_latest_task_onMachine[m_id_for_action] = op_id
                ops_mat[job_id].pop(0)
            adj_mat_mc = np.pad(
                adj_mat_mc, ((1, 1), (1, 1)), "constant", constant_values=0
            ...
            dur_mat = np.pad(
                dur_mat.reshape(-1, 1), ((1, 1), (0, 0)), "constant", constant_values=0
            ).repeat(self.n_oprs + 2, axis=1)
            edge_weight = np.multiply(adj_all, dur_mat)
            G = nx.from_numpy_matrix(
                edge_weight, parallel_edges=False, create_using=nx.DiGraph
            )  # create nx.DiGraph
            G.add_weighted_edges_from(
                [(0, i, 0) for i in range(1, self.n_oprs + 2 - 1, self.n_mch)]
            )  # add release time, here all jobs are available at t=0. This is the only way to add release date. And if you do not add release date, startime computation will return wired value
            current_graphs.append(G)
            G_mc = nx.from_numpy_matrix(
                adj_mat_mc, parallel_edges=False, create_using=nx.DiGraph
            )  # create nx.DiGraph
            ...
            edge_indices_mc.append(
                (torch.nonzero(torch.from_numpy(adj_mat_mc)).t().contiguous())
                + (n_operations + 2) * i
            )
        ...
        if self.evaluator_type == "message-passing":
            est, lst, make_span = self.eva.forward(
                edge_index=torch.cat([edge_indices_pc, edge_indices_mc], dim=-1),
                duration=durations,
                n_j=self.n_job,
                n_m=self.n_mch,
            )
        ...
        x = torch.cat(
            [
                durations / self.high,
                est / self.fea_norm_const,
                lst / self.fea_norm_const,
            ],
            dim=-1,
        )
        # prepare batch
        batch = torch.from_numpy(
            np.repeat(
                np.arange(instances.shape[0], dtype=np.int64),
                repeats=self.n_job * self.n_mch + 2,
            )
        ).to(device)

        return (
            (x, edge_indices_pc, edge_indices_mc, batch),
            current_graphs,
            sub_graphs_mc,
            make_span,
        )
    ...
    def _rules_solver(self, args, plot=False):
        ...
        for i, instance in enumerate(instances):
            ...
            n_operations = n_jobs * n_machines
            last_col = np.arange(start=0, stop=n_operations, step=1).reshape(
                n_jobs, -1
            )[:, -1]
            candidate_oprs = np.arange(start=0, stop=n_operations, step=1).reshape(
                n_jobs, -1
            )[
                :, 0
            ]  # initialize action space: [n_jobs, 1], the first column
            mask = np.zeros(
                shape=n_jobs, dtype=bool
            )  # initialize the mask: [n_jobs, 1]
            adj_mat_mc = np.zeros(
                shape=[n_operations, n_operations], dtype=int
            )  # Create adjacent matrix for machine clique
            ...
            for _ in range(n_operations):
                ...
                if rule_type == "spt":
                    ...
                    idx = np.random.choice(
                        np.where(dur_candidate == np.min(dur_candidate))[0]
                    )
                    ...
                    wkr = np.take(
                        np.cumsum(np.multiply(dur_mat, 1 - finished_mark), axis=1),
                        last_col[np.where(~mask)],
                    )
                    priority = fdd / wkr
    ...
    def dag2pyg(self, instances, nx_graphs, device):
        ...
        for i, (instance, G_mc) in enumerate(zip(instances, nx_graphs)):
            durations.append(
                np.pad(instance[0].reshape(-1), (1, 1), "constant", constant_values=0)
            )
        ...
        edge_indices_mc = torch.cat(edge_indices_mc, dim=-1).to(device)
        durations = (
            torch.from_numpy(np.concatenate(durations)).reshape(-1, 1).to(device)
        )
        if self.evaluator_type == "message-passing":
            est, lst, make_span = self.eva.forward(
                edge_index=torch.cat([edge_indices_pc, edge_indices_mc], dim=-1),
    ...
    def change_nxgraph_topology(self, actions, plot=False):
        ...
        for i, (action, G, G_mc, instance) in enumerate(
            zip(actions, self.current_graphs, self.sub_graphs_mc, self.instances)
        ):
            if action == [0, 0]:  # if dummy action then do not transit
                ...
                S = [
                    s
                    for s in G.predecessors(action[0])
                    if int((s - 1) // n_machines) != int((action[0] - 1) // n_machines)
                    and s != 0
                ]
                T = [
                    t
                    for t in G.successors(action[1])
                    if int((t - 1) // n_machines) != int((action[1] - 1) // n_machines)
                    and t != n_operations + 1
                ]
                ...
                if t is not None:  # connect action[0] with t
                    ...
                    G_mc.add_edge(
                        action[0], t, weight=np.take(instance[0], action[0] - 1)
                    )
                else:
                    pass
                ...
                G.add_edge(
                    action[1], action[0], weight=np.take(instance[0], action[1] - 1)
                )
    ...
    def step(self, actions, device, plot=False):
        ...
        self.incumbent_objs = torch.where(
            makespan - self.incumbent_objs < 0, makespan, self.incumbent_objs
        )
        ...
        feasible_actions, flag = self.feasible_actions(
            device
        )  # new feasible actions w.r.t updated tabu list
    ...
    def reset(self, instances, init_type, device, plot=False):
        ...
        if init_type == "plist":
            plist = np.repeat(
                np.random.permutation(np.arange(self.n_job).repeat(self.n_mch)).reshape(
                    1, -1
                ),
            ...
            (
                ...
                sub_graphs_mc,
                make_span,
            ) = self._p_list_solver(args=[self.instances, plist, device], plot=plot)
        elif init_type == "spt":
            (
                (x, edge_indices_pc, edge_indices_mc, batch),
                current_graphs,
                sub_graphs_mc,
                make_span,
            ) = self._rules_solver(args=[self.instances, device, "spt"], plot=plot)
    ...
    def feasible_actions(self, device):
        ...
        for i, (G, instance, tabu_list) in enumerate(
            zip(self.current_graphs, self.instances, self.tabu_lists)
        ):
            action = self._gen_moves(
                solution=G, mch_mat=instance[1], tabu_list=tabu_list
            )
            # print(action)
            if len(action) != 0:
                actions.append(action)
